...
module.exports.import = function(patientId, src, callback){
  ...
  var shalls = collectShalls(ret);

  validator(shalls, function(err, codes){
    ret.codes = codes;
    console.log(Object.keys(codes).length, "codes fetched");
    ret.cleanupTree(1); // 1st, then
    ret.cleanupTree("paredown"); // 2nd-line cleanup 
