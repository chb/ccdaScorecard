...
Component.cleanupStep(Cleanup.clearNulls, "paredown").withNegationStatus(false);
  ...
  common.deepForEach(ccda, {
    pre: function(v){
      if (v instanceof Component){
        if (v.constructor.shallConditions){
          shalls.push({data: v.js, conditions: v.constructor.shallConditions});
        }
        return v.js
      }
      return v;
...
CCDA.prototype.asyncRun = function(node, callback){
  var self = this;
  this.errors = [];
  this.run.apply(this, arguments);
  this.cleanupTree(1); // 1st, then 
  this.cleanupTree("paredown"); // 2nd-line cleanup 

  var validator = require("../ccda-validation");
  var shalls = collectShalls(this);

  console.log(shalls);
  validator(shalls, function(err, codes){
    console.log(self);
    callback(err, self);
    console.log(codes);
  });
...
module.exports.import = function(patientId, src, callback){
  var ret = new CCDA();
  var xml = lxml.parseXmlString(src);
  ret.patientId = patientId;
  ret.src = src;
  ret.asyncRun(xml, callback);
};
