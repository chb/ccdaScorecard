 ...
 * @api public
 */
module.exports = function gradeCcda(src, done){
  var manager = new Manager(src);
...
function Vocab(codes){
  this.codes = codes;
}

// yuck - linerar comparison across all codes in the set :-)
// TODO replace with code lookup hash.
Vocab.prototype.lookup = function(code){
  var needMatchOn = {
    "code": "conceptCode",
    "system": "codeSystemOid"
  };

  if (code.expectedValueSet){
    needMatchOn.expectedValueSet = "valueSetOid";
  }
  var matches = this.codes.filter(function(c){
    var ret = true;
    Object.keys(needMatchOn).forEach(function(k){
      if (code[k] !== c[needMatchOn[k]]) {
        ret = false;
      }
    });
    return ret;
...
function Manager(src){
  this.ccdaSrc = src;
  this.ccda = lxml.parseXmlString(src);
...
Manager.prototype._fetchVocab = function(done, results) {
  ...
  db.vocab.collection("valueSetConcepts", function(err, concepts){
    if (err){ 
      return done(err); 
    }
    var orq = codes.map(function(c){
      return {
        "conceptCode": c.code,
        "codeSystemOid": c.system
      };
    });

    var q = {"$or": orq};
    concepts.find(q).toArray(function(err, vals){
      manager.vocab = new Vocab(vals);
...
Manager.prototype._logger = function(done, results) {
  ...
  db.ccdaScorecard.collection("ccdas", function(err, ccdas){
    ...
    ccdas.insert({raw: manager.ccdaSrc}, {safe: false});
  }); 
...
function updateBenchmarks(scores){
  db.vocab.collection("scoreStats", function(err, stats){
    if (err){ 
      return done(err); 
    }
